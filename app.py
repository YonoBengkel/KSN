...
tab_plot, tab_denah = st.tabs(['📊 Plotting Jadwal', '🏢 Cetak Denah Ruangan'])
...
with tab_denah:
    st.header("🏢 Cetak Denah Ruangan")
    st.markdown("Unggah file **Jadwal Excel** yang sudah jadi (Misal: *Salinan dari Jadwal & Denah Bikom 5 Dataset.xlsx*) untuk menghasilkan *template* Word ruangan siap cetak.")
    
    file_jadwal = st.file_uploader("Upload File Jadwal Excel", type=["xlsx"], key="jadwal")
    
    if file_jadwal:
        try:
            excel_file = pd.ExcelFile(file_jadwal)
            sheet_names = excel_file.sheet_names
            
            # Filter sheet yang mungkin berisi jadwal (contoh: mengandung kata 'Jadwal')
            jadwal_sheets = [s for s in sheet_names if 'jadwal' in s.lower()]
            if not jadwal_sheets:
                jadwal_sheets = sheet_names # Jika tidak ada, tampilkan semua
                
            selected_sheets = st.multiselect("Pilih Tab / Sheet yang akan diproses", sheet_names, default=jadwal_sheets)
            
            if st.button("📄 Generate Denah DOCX", type="primary", use_container_width=True) and selected_sheets:
                with st.spinner("Sedang memproses dan membuat file DOCX..."):
                    doc = docx.Document()
                    
                    for sheet in selected_sheets:
                        df = pd.read_excel(file_jadwal, sheet_name=sheet, header=None)
                    ...
                    # Hapus page break terakhir (workaround docx hapus text)
                    ...
                    st.success("✅ File DOCX berhasil dibuat!")
                    
                    st.download_button(
                        label="⬇️ Download Denah Ruangan DOCX",
                        data=docx_buffer,
                        file_name="Denah_Ruangan_Bikom.docx",
